  ...
  while (1)
  {
    ...
    LL_IWDG_ReloadCounter(IWDG);
	  ...
	  if(isScreenReady() && ADXL345hasChanged(X_AXIS))
		  SSD1306_printAngle(getAngleDegrees(X_AXIS), SSD1306_LINE1_PAGE, SSD1306_LINE1_COLUMN);

	  //if Y axis angle changed, update the screen
	  if(isScreenReady() && ADXL345hasChanged(Y_AXIS))
		  SSD1306_printAngle(getAngleDegrees(Y_AXIS), SSD1306_LINE2_PAGE, SSD1306_LINE2_COLUMN);
    /* USER CODE END WHILE */
