 ...
 *
 * while (1) {
 *     float accel[kNBaxis], gyro[kNBaxis];
 ...
 *
 * @param[out] context          Current Mahony filter context
 * @param[in] accelerometer_g   Accelerometer measurements in [G] (9.81 m/s²)
 * @param[in] gyroscope_radps   Gyroscope measurements in [rad/s]
 */
void updateMahonyFilter(MahonyContext* context,
                        const float accelerometer_g[kNBaxis],  //NOLINT(bugprone-easily-swappable-parameters)
    ...
    float corrected_gyro_radps[kNBaxis] = {[kXaxis] = (gyroscope_radps[kXaxis] + (context->kp * errors[kXaxis])),
                                           [kYaxis] = (gyroscope_radps[kYaxis] + (context->kp * errors[kYaxis])),
                                           [kZaxis] = (gyroscope_radps[kZaxis] + (context->kp * errors[kZaxis]))};
...
static inline FORCE_INLINE_SILENT float getDT(const TimeDelta* delta) {
    ...
    return ((float)delta_ticks * delta->resolution_seconds);
}
