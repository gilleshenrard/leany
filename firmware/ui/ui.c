 ...
 */
typedef enum {
    kPrintMeasurements = 1,  ///< printMeasurements() function
    kPrintCharacter = 2,     ///< printCharacter() function
...
static ErrorCode printMeasurements(Axis axis, int16_t angle_degrees_tenths) {
    ...
    EXIT_ON_ERROR(result, kPrintMeasurements, 1)
...
static ErrorCode printCharacter(VerdanaCharacter character, uint8_t x_left, uint8_t y_top) {
    ...
    EXIT_ON_ERROR(result, kPrintCharacter, 1)
    return result;
}
