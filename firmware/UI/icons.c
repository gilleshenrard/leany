/*
 * SPDX-FileCopyrightText: 2025 Gilles Henrard <contact@gilleshenrard.com>
 *
 * SPDX-License-Identifier: MIT
 */

/**
 * @file icons.c
 * @author Gilles Henrard
 * @brief Declare icons bitmaps where each bit represent either a background or a foreground pixel
 * @date 25/07/2025
 * 
 * @copyright Copyright (c) 2024
 * 
 *  * @details Generated with the application The Dot Factory v0.1.4
 * 
 * Software Github page : https://github.com/pavius/the-dot-factory
 *
 * 1. generate bitmaps
 *      - characters : 0123456789+-.
 *      - Verdana 36pts
 *      - Height and width fixed
 *      - Line wrap at column
 *      - Do not generate descriptors
 *      - Comments : Bitmap visualiser and char descriptor
 *      - Byte : RowMajor, MSB first, Format Hex, leading 0x
 *      - Variable format : Bitmaps "const uint32_t {0}Bitmaps"
 * 2. copy bitmaps only
 * 3. remove all ", 0x" to merge row bytes together
 * 4. enclose each bitmap with carrets and a comma
 * 5. transform the array in a two-dimensions array
 * 6. move numbers at first places to take advantage of indexes in the array
 * 7. assign each bitmap array to an enum entry
 */

#include "icons.h"

#include <stdint.h>

/**
 * @brief Type definition of a line of a character
 */
typedef uint32_t CharacterLine;

/**
 * @brief Values a bit in the bitmaps can have
 */
typedef enum {
    kBackground = 0,  ///< Background = 0
    kForeground,      ///< Foreground = 1
} PixelType;

/**
 * @brief Characters array forward-declaration
 */
extern const CharacterLine kVerdana36ptBitmaps[kNbVerdataCharacters][kVerdanaNbRows];

/**
 * @brief Uncompress a row of a character from the array to a buffer
 * @note Each character is represented with a bit for each pixel (values are either background or foreground)
 * 
 * @param[out] buffer Buffer to fill with the pixel values
 * @param character Character to uncompress
 */
void uncompressCharacter(Pixel buffer[], VerdanaCharacter character) {
    for (uint8_t row = 0; row < (uint8_t)kVerdanaNbRows; row++) {
        for (uint8_t column = 0; column < (uint8_t)kVerdanaNbColumns; column++) {
            //create a mask showing the nature of the current pixel (foreground or background)
            CharacterLine pixel_mask = (CharacterLine)1U << ((CharacterLine)kVerdanaNbColumns - column + 2U - 1U);

            //get the proper colour to fill
            Pixel colour = ((kVerdana36ptBitmaps[character][row] & pixel_mask) == kBackground ? kDarkCharcoalBigEndian
                                                                                              : kBrightGrayBigEndian);

            //set the pixel colour in the buffer
            buffer[(row * kVerdanaNbColumns) + column] = colour;
        }
    }
}

/**
 * @brief Array of character bitmaps, where each pixel is described as a bit (foreground or background)
 */
const CharacterLine kVerdana36ptBitmaps[kNbVerdataCharacters][kVerdanaNbRows] = {
    // @444 '0' (30 pixels wide)
    [kVerdana0] =
        {
            0x001FC000,  //            #######
            0x00FFF800,  //         #############
            0x01FFFC00,  //        ###############
            0x03FFFE00,  //       #################
            0x07F07F00,  //      #######     #######
            0x0FC01F80,  //     ######         ######
            0x0F800F80,  //     #####           #####
            0x0F800F80,  //     #####           #####
            0x1F0007C0,  //    #####             #####
            0x1F0007C0,  //    #####             #####
            0x1F0007C0,  //    #####             #####
            0x1E0003C0,  //    ####               ####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x1E0003C0,  //    ####               ####
            0x1F0007C0,  //    #####             #####
            0x1F0007C0,  //    #####             #####
            0x1F0007C0,  //    #####             #####
            0x0F800F80,  //     #####           #####
            0x0F800F80,  //     #####           #####
            0x0FC01F80,  //     ######         ######
            0x07F07F00,  //      #######     #######
            0x03FFFE00,  //       #################
            0x01FFFC00,  //        ###############
            0x00FFF800,  //         #############
            0x001FC000,  //            #######
        },
    // @592 '1' (30 pixels wide)
    [kVerdana1] =
        {
            0x00000000,  //
            0x0003C000,  //               ####
            0x0003C000,  //               ####
            0x0007C000,  //              #####
            0x000FC000,  //             ######
            0x003FC000,  //           ########
            0x03FFC000,  //       ############
            0x03FFC000,  //       ############
            0x03FFC000,  //       ############
            0x03FFC000,  //       ############
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x0007C000,  //              #####
            0x03FFFF80,  //       ###################
            0x03FFFF80,  //       ###################
            0x03FFFF80,  //       ###################
            0x03FFFF80,  //       ###################
            0x00000000,  //
        },
    // @740 '2' (30 pixels wide)
    [kVerdana2] =
        {
            0x007FE000,  //          ##########
            0x03FFF800,  //       ###############
            0x0FFFFE00,  //     ###################
            0x0FFFFF00,  //     ####################
            0x0FC07F80,  //     ######       ########
            0x0E001F80,  //     ###            ######
            0x08000FC0,  //     #               ######
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x00000F80,  //                     #####
            0x00000F80,  //                     #####
            0x00001F80,  //                    ######
            0x00001F00,  //                    #####
            0x00003E00,  //                   #####
            0x00007E00,  //                  ######
            0x0000FC00,  //                 ######
            0x0001F800,  //                ######
            0x0003F000,  //               ######
            0x0007E000,  //              ######
            0x000FC000,  //             ######
            0x001F8000,  //            ######
            0x003F0000,  //           ######
            0x00FE0000,  //         #######
            0x01F80000,  //        ######
            0x03F00000,  //       ######
            0x07E00000,  //      ######
            0x0FC00000,  //     ######
            0x1F800000,  //    ######
            0x1FFFFFE0,  //    ########################
            0x1FFFFFE0,  //    ########################
            0x1FFFFFE0,  //    ########################
            0x1FFFFFE0,  //    ########################
            0x00000000,  //
        },
    // @888 '3' (30 pixels wide)
    [kVerdana3] =
        {
            0x007FC000,  //          #########
            0x03FFF800,  //       ###############
            0x0FFFFC00,  //     ##################
            0x0FFFFE00,  //     ###################
            0x0FC07F00,  //     ######       #######
            0x0E001F00,  //     ###            #####
            0x08001F80,  //     #              ######
            0x00000F80,  //                     #####
            0x00000F80,  //                     #####
            0x00000F80,  //                     #####
            0x00000F80,  //                     #####
            0x00000F00,  //                     ####
            0x00001F00,  //                    #####
            0x00003E00,  //                   #####
            0x0000FC00,  //                 ######
            0x001FF800,  //            ##########
            0x001FC000,  //            #######
            0x001FF800,  //            ##########
            0x001FFE00,  //            ############
            0x00007F00,  //                  #######
            0x00001F80,  //                    ######
            0x00000F80,  //                     #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x00000F80,  //                     #####
            0x10001F80,  //    #               ######
            0x1C003F00,  //    ###            ######
            0x1F80FF00,  //    ######       ########
            0x1FFFFE00,  //    ####################
            0x1FFFFC00,  //    ###################
            0x07FFF000,  //      ###############
            0x00FFC000,  //         ##########
        },
    // @1036 '4' (30 pixels wide)
    [kVerdana4] =
        {
            0x00000000,  //
            0x00007C00,  //                  #####
            0x0000FC00,  //                 ######
            0x0001FC00,  //                #######
            0x0003FC00,  //               ########
            0x0007FC00,  //              #########
            0x0007FC00,  //              #########
            0x000FFC00,  //             ##########
            0x001F7C00,  //            ##### #####
            0x003E7C00,  //           #####  #####
            0x007E7C00,  //          ######  #####
            0x007C7C00,  //          #####   #####
            0x00F87C00,  //         #####    #####
            0x01F07C00,  //        #####     #####
            0x03E07C00,  //       #####      #####
            0x07C07C00,  //      #####       #####
            0x0FC07C00,  //     ######       #####
            0x0F807C00,  //     #####        #####
            0x1F007C00,  //    #####         #####
            0x3E007C00,  //   #####          #####
            0x7C007C00,  //  #####           #####
            0x78007C00,  //  ####            #####
            0x7FFFFFE0,  //  ##########################
            0x7FFFFFE0,  //  ##########################
            0x7FFFFFE0,  //  ##########################
            0x7FFFFFE0,  //  ##########################
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00007C00,  //                  #####
            0x00000000,  //
        },
    // @1184 '5' (30 pixels wide)
    [kVerdana5] =
        {
            0x07FFFFE0,  //      ######################
            0x07FFFFE0,  //      ######################
            0x07FFFFE0,  //      ######################
            0x07FFFFE0,  //      ######################
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07C00000,  //      #####
            0x07FFE000,  //      ##############
            0x07FFFC00,  //      #################
            0x07FFFE00,  //      ##################
            0x07FFFF00,  //      ###################
            0x06007F80,  //      ##          ########
            0x00001FC0,  //                    #######
            0x000007C0,  //                      #####
            0x000007E0,  //                      ######
            0x000003E0,  //                       #####
            0x000003E0,  //                       #####
            0x000003E0,  //                       #####
            0x000003E0,  //                       #####
            0x000003E0,  //                       #####
            0x000003E0,  //                       #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x10000FC0,  //    #                ######
            0x1C001F80,  //    ###             ######
            0x1F807F00,  //    ######        #######
            0x1FFFFF00,  //    #####################
            0x1FFFFC00,  //    ###################
            0x0FFFF800,  //     #################
            0x00FFC000,  //         ##########
        },
    // @1332 '6' (30 pixels wide)
    [kVerdana6] =
        {
            0x0001FF00,  //                #########
            0x000FFF00,  //             ############
            0x003FFF00,  //           ##############
            0x007FFF00,  //          ###############
            0x01FE0300,  //        ########       ##
            0x01F80000,  //        ######
            0x03F00000,  //       ######
            0x07E00000,  //      ######
            0x07C00000,  //      #####
            0x0F800000,  //     #####
            0x0F800000,  //     #####
            0x1F000000,  //    #####
            0x1F000000,  //    #####
            0x1F000000,  //    #####
            0x1E1FF000,  //    ####    #########
            0x3E7FFC00,  //   #####  #############
            0x3FFFFE00,  //   #####################
            0x3FFFFF00,  //   ######################
            0x3FC03F80,  //   ########        #######
            0x3F000FC0,  //   ######            ######
            0x3E0007C0,  //   #####              #####
            0x3E0007E0,  //   #####              ######
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x1E0003E0,  //    ####               #####
            0x1F0003E0,  //    #####              #####
            0x1F0003E0,  //    #####              #####
            0x1F8007C0,  //    ######            #####
            0x0F800FC0,  //     #####           ######
            0x0FC00F80,  //     ######          #####
            0x07F03F80,  //      #######      #######
            0x03FFFF00,  //       ##################
            0x01FFFC00,  //        ###############
            0x007FF800,  //          ############
            0x001FC000,  //            #######
        },
    // @1480 '7' (30 pixels wide)
    [kVerdana7] =
        {
            0x00000000,  //
            0x1FFFFFE0,  //    ########################
            0x1FFFFFE0,  //    ########################
            0x1FFFFFE0,  //    ########################
            0x1FFFFFE0,  //    ########################
            0x000003E0,  //                       #####
            0x000007E0,  //                      ######
            0x000007C0,  //                      #####
            0x00000FC0,  //                     ######
            0x00000F80,  //                     #####
            0x00001F80,  //                    ######
            0x00001F00,  //                    #####
            0x00003F00,  //                   ######
            0x00003E00,  //                   #####
            0x00007E00,  //                  ######
            0x00007C00,  //                  #####
            0x0000FC00,  //                 ######
            0x0000F800,  //                 #####
            0x0001F800,  //                ######
            0x0001F000,  //                #####
            0x0003F000,  //               ######
            0x0003E000,  //               #####
            0x0007E000,  //              ######
            0x0007C000,  //              #####
            0x000FC000,  //             ######
            0x001F8000,  //            ######
            0x001F8000,  //            ######
            0x003F0000,  //           ######
            0x003F0000,  //           ######
            0x007E0000,  //          ######
            0x007E0000,  //          ######
            0x00FC0000,  //         ######
            0x00F80000,  //         #####
            0x01F80000,  //        ######
            0x01F00000,  //        #####
            0x03F00000,  //       ######
            0x00000000,  //
        },
    // @1628 '8' (30 pixels wide)
    [kVerdana8] =
        {
            0x003FE000,  //           #########
            0x00FFF800,  //         #############
            0x03FFFE00,  //       #################
            0x07FFFF00,  //      ###################
            0x07E07F80,  //      ######      ########
            0x0FC01F80,  //     ######         ######
            0x1F800FC0,  //    ######           ######
            0x1F0007C0,  //    #####             #####
            0x1F0007C0,  //    #####             #####
            0x1F0007C0,  //    #####             #####
            0x1F0007C0,  //    #####             #####
            0x1F8007C0,  //    ######            #####
            0x0FC00F80,  //     ######          #####
            0x0FE00F80,  //     #######         #####
            0x07F81F00,  //      ########      #####
            0x03FE3E00,  //       #########   #####
            0x01FFFC00,  //        ###############
            0x00FFF000,  //         ############
            0x03EFFC00,  //       ##### ##########
            0x07C1FF00,  //      #####     #########
            0x0F807F80,  //     #####        ########
            0x1F001FC0,  //    #####           #######
            0x1F000FC0,  //    #####            ######
            0x3E0007E0,  //   #####              ######
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3F0007E0,  //   ######             ######
            0x1F8007C0,  //    ######            #####
            0x1FC00FC0,  //    #######          ######
            0x0FF03F80,  //     ########      #######
            0x07FFFF00,  //      ###################
            0x03FFFE00,  //       #################
            0x00FFF800,  //         #############
            0x003FE000,  //           #########
        },
    // @1776 '9' (30 pixels wide)
    [kVerdana9] =
        {
            0x001FC000,  //            #######
            0x00FFF800,  //         #############
            0x01FFFC00,  //        ###############
            0x07FFFE00,  //      ##################
            0x0FE07F00,  //     #######      #######
            0x0FC01F80,  //     ######         ######
            0x1F800F80,  //    ######           #####
            0x1F000FC0,  //    #####            ######
            0x3E0007C0,  //   #####              #####
            0x3E0007C0,  //   #####              #####
            0x3E0003C0,  //   #####               ####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3E0003E0,  //   #####               #####
            0x3F0003E0,  //   ######              #####
            0x1F0003E0,  //    #####              #####
            0x1F8007E0,  //    ######            ######
            0x0FE03FE0,  //     #######       #########
            0x07FFFFE0,  //      ######################
            0x03FFFFE0,  //       #####################
            0x01FFF3E0,  //        #############  #####
            0x007FC3C0,  //          #########    ####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x000007C0,  //                      #####
            0x00000F80,  //                     #####
            0x00000F80,  //                     #####
            0x00001F00,  //                    #####
            0x00001F00,  //                    #####
            0x00003E00,  //                   #####
            0x0000FE00,  //                 #######
            0x0603FC00,  //      ##       ########
            0x07FFF000,  //      ###############
            0x07FFE000,  //      ##############
            0x07FF8000,  //      ############
            0x07FC0000,  //      #########
        },
    // @0 '+' (30 pixels wide)
    [kVerdanaPlus] =
        {
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0xFFFFFFFC,  // ##############################
            0xFFFFFFFC,  // ##############################
            0xFFFFFFFC,  // ##############################
            0xFFFFFFFC,  // ##############################
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00078000,  //              ####
            0x00000000,  //
        },
    // @148 '-' (30 pixels wide)
    [kVerdanaMinus] =
        {
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00FFFE00,  //         ###############
            0x00FFFE00,  //         ###############
            0x00FFFE00,  //         ###############
            0x00FFFE00,  //         ###############
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
        },
    // @296 '.' (30 pixels wide)
    [kVerdanaDot] =
        {
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x00000000,  //
            0x0007E000,  //              ######
            0x0007E000,  //              ######
            0x0007E000,  //              ######
            0x0007E000,  //              ######
            0x0007E000,  //              ######
            0x0007E000,  //              ######
            0x0007E000,  //              ######
            0x00000000,  //
        },
};
