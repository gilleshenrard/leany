...
typedef enum {
    ...
    kSPIwriteRegistersBunch = 4U,     ///< Function ID for the writeRegistersBunch()
} FunctionCode;
...
/**
 * Sequentially Write an array of register number/value pairs
 *
 * @param descriptor SPI descriptor to use
 ...
 */
ErrorCode writeRegistersBunch(SPI* descriptor, const uint8_t registers[][2], const uint8_t nb_registers) {
    ErrorCode result = kSuccessCode;

    uint8_t reg = 0;
    do {
        result = writeRegisters(descriptor, registers[reg][0], &registers[reg][1], 1);
        reg++;
    } while ((reg < nb_registers) && !isError(result));
    EXIT_ON_ERROR(result, kSPIwriteRegistersBunch, 1)
