/**
 * SPDX-FileCopyrightText: 2026 Gilles Henrard <contact@gilleshenrard.com>
 * SPDX-License-Identifier: MIT
 * 
 * @file hal_i2c.c
 * @brief Implement generic I²C transmission
 * @author Gilles Henrard
 */
#include "hal_i2c.h"

#include <stdint.h>
#include <stm32f103xb.h>
#include <stm32f1xx_ll_i2c.h>

#include "errorstack.h"
#include "systick.h"

enum {
    kI2Ctimeout_ms = 20U,  ///< Maximum number of milliseconds an I²C transfer can last
    kREAD = 0x01U,         ///< Bit value to apply to read data from the charger
    kWRITE = 0x00U,        ///< Bit value to apply to write data to the charger
};

/**
 * @brief Enumeration of all the function ID used in errors
 */
typedef enum {
    kReadRegisters = 1,      ///< readRegisters() function
    kInitiateReception = 2,  ///< initiateReception() function
    kWriteRegisters = 3,     ///< writeRegisters() function
    kCheckErrors = 4,        ///< checkI2CFlags() function
} FunctionCode;

static ErrorCode initiateTransaction(I2C_TypeDef* descriptor, uint8_t slave_address, uint8_t first_register);
static ErrorCode checkI2CFlags(I2C_TypeDef* descriptor, uint32_t start_tick, uint32_t timeout_ms);

/**
 * @brief Poll an I²C flag until set or checkI2CFlags() reports a fault
 */
#define EXIT_ON_I2C_ERROR(condition, descriptor, function_id, timeout_ms, error_code) \
    while (!(condition)) {                                                            \
        if (isError(checkI2CFlags(descriptor, start_tick, timeout_ms))) {             \
            return pushErrorCode(result, function_id, error_code);                    \
        }                                                                             \
    }

/*********************************************************************************************************************************/
/*********************************************************************************************************************************/

/**
 * Burst read data via I²C
 *
 * @param descriptor I²C descriptor to use
 * @param slave_address I²C slave address
 * @param first_register First register to read
 * @param[out] data Data buffer to fill
 * @param nb_bytes Number of bytes to read
 * @retval 0 Success
 * @retval 1 Invalid descriptor or data buffer provided
 * @retval 2 Error while initiating the read sequence
 * @retval 3 Timeout while waiting for the data byte
 * @retval 4 Timeout while waiting for slave address to be sent
 * @retval 5 Timeout while waiting for a data byte to be received
 * @retval 6 Timeout while waiting for the last data byte to be received
 */
ErrorCode readI2Cregisters(I2C_TypeDef* descriptor, uint8_t slave_address, uint8_t first_register, uint8_t data[],
                           uint8_t nb_bytes) {
    if (!descriptor || (!data && nb_bytes)) {
        return createErrorCode(kReadRegisters, 1, kErrorError);
    }

    if (!nb_bytes) {
        return kSuccessCode;
    }

    //initiate the I²C reception
    ErrorCode result = initiateTransaction(descriptor, slave_address, first_register);
    EXIT_ON_ERROR(result, kReadRegisters, 2)

    uint32_t start_tick = getCurrentTick();

    //resend the slave address in read mode
    LL_I2C_GenerateStartCondition(descriptor);
    EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_SB(descriptor), descriptor, kI2Ctimeout_ms, kInitiateReception, 3)
    LL_I2C_TransmitData8(descriptor, (uint8_t)(slave_address << 1U) | (uint8_t)kREAD);
    EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_ADDR(descriptor), descriptor, kI2Ctimeout_ms, kInitiateReception, 4)
    LL_I2C_ClearFlag_ADDR(descriptor);

    //read all bytes except for the last one
    uint8_t current_byte = 0;
    LL_I2C_AcknowledgeNextData(descriptor, LL_I2C_ACK);
    while (nb_bytes - 1U) {
        EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_RXNE(descriptor), descriptor, kI2Ctimeout_ms, kReadRegisters, 5)
        data[current_byte] = LL_I2C_ReceiveData8(descriptor);
        current_byte++;
        nb_bytes--;
    }

    //read the last byte, with a NACK and a STOP signal
    LL_I2C_AcknowledgeNextData(descriptor, LL_I2C_NACK);
    LL_I2C_GenerateStopCondition(descriptor);
    EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_RXNE(descriptor), descriptor, kI2Ctimeout_ms, kReadRegisters, 6)
    data[current_byte] = LL_I2C_ReceiveData8(descriptor);

    EXIT_ON_I2C_ERROR(!LL_I2C_IsActiveFlag_BUSY(descriptor), descriptor, kI2Ctimeout_ms, kReadRegisters, 7)
    return kSuccessCode;
}

/**
 * Burst-write data to registers
 *
 * @param descriptor I²C descriptor to use
 * @param slave_address I²C slave address
 * @param first_register Number of the first register to which write data
 * @param data Data to write
 * @param nb_bytes Number of bytes of data to write
 * @retval 0 Success
 * @retval 1 Invalid descriptor or data buffer provided
 * @retval 2 Error while initiating the write sequence
 * @retval 3 Timeout while waiting for TXE to be set
 * @retval 4 Timeout while waiting for TXE to be set before the last data byte
 * @retval 5 Timeout while waiting for Byte Transfer Finish to be set
 */
ErrorCode writeI2CRegisters(I2C_TypeDef* descriptor, uint8_t slave_address, uint8_t first_register,
                            const uint8_t data[], uint8_t nb_bytes) {
    if (!descriptor || (!data && nb_bytes)) {
        return createErrorCode(kWriteRegisters, 1, kErrorError);
    }

    if (!nb_bytes) {
        return kSuccessCode;
    }

    //initiate the I²C transmission
    ErrorCode result = initiateTransaction(descriptor, slave_address, first_register);
    EXIT_ON_ERROR(result, kWriteRegisters, 2)

    uint32_t start_tick = getCurrentTick();

    //send the data bytes all until the one before last
    uint8_t current_byte = 0;
    while (nb_bytes - 1U) {
        EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_TXE(descriptor), descriptor, kI2Ctimeout_ms, kWriteRegisters, 3)
        LL_I2C_TransmitData8(descriptor, data[current_byte]);
        current_byte++;
        nb_bytes--;
    }

    //send the last byte and stop the transmission
    EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_TXE(descriptor), descriptor, kI2Ctimeout_ms, kWriteRegisters, 4)
    LL_I2C_TransmitData8(descriptor, data[current_byte]);
    EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_BTF(descriptor), descriptor, kI2Ctimeout_ms, kWriteRegisters, 5)
    LL_I2C_GenerateStopCondition(descriptor);

    EXIT_ON_I2C_ERROR(!LL_I2C_IsActiveFlag_BUSY(descriptor), descriptor, kI2Ctimeout_ms, kWriteRegisters, 6)

    return kSuccessCode;
}

/*********************************************************************************************************************************/
/*********************************************************************************************************************************/

/**
 * Initiate an I²C reception sequence
 *
 * @param descriptor I²C descriptor to use
 * @param slave_address I²C slave address
 * @param first_register Number of the first register to read
 * @retval 0 Success
 * @retval 1 Timeout while waiting for start signal to be done
 * @retval 2 Timeout while waiting for ADDR byte to be sent
 * @retval 3 Timeout while waiting for the register address to be sent
 */
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
static ErrorCode initiateTransaction(I2C_TypeDef* descriptor, uint8_t slave_address, uint8_t first_register) {
    uint32_t start_tick = getCurrentTick();

    //send a start signal
    LL_I2C_GenerateStartCondition(descriptor);
    EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_SB(descriptor), descriptor, kI2Ctimeout_ms, kInitiateReception, 1)

    //send the slave address
    LL_I2C_TransmitData8(descriptor, (uint8_t)(slave_address << 1U) | (uint8_t)kWRITE);
    EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_ADDR(descriptor), descriptor, kI2Ctimeout_ms, kInitiateReception, 2)
    LL_I2C_AcknowledgeNextData(descriptor, LL_I2C_NACK);
    LL_I2C_ClearFlag_ADDR(descriptor);

    //send the register address
    EXIT_ON_I2C_ERROR(LL_I2C_IsActiveFlag_TXE(descriptor), descriptor, kI2Ctimeout_ms, kInitiateReception, 3)
    LL_I2C_TransmitData8(descriptor, first_register);

    return kSuccessCode;
}

/**
 * Check the I²C error flags during a transmission
 *
 * @param descriptor I²C descriptor
 * @param start_tick Tick at the start of the transmission
 * @param timeout_ms Maximum number of milliseconds before a timeout
 * @retval 0 Success
 * @retval 1 AF flag
 * @retval 2 ARLO flag
 * @retval 3 BERR flag
 * @retval 4 Timeout
 */
static ErrorCode checkI2CFlags(I2C_TypeDef* descriptor, uint32_t start_tick, uint32_t timeout_ms) {
    if (LL_I2C_IsActiveFlag_AF(descriptor)) {
        LL_I2C_ClearFlag_AF(descriptor);
        LL_I2C_GenerateStopCondition(descriptor);
        return createErrorCode(kCheckErrors, kErrorAcknowledgeFailure, kErrorError);
    }

    if (LL_I2C_IsActiveFlag_ARLO(descriptor)) {
        LL_I2C_ClearFlag_ARLO(descriptor);
        return createErrorCode(kCheckErrors, kErrorArbitrationLost, kErrorError);
    }

    if (LL_I2C_IsActiveFlag_BERR(descriptor)) {
        LL_I2C_ClearFlag_BERR(descriptor);
        LL_I2C_GenerateStopCondition(descriptor);
        return createErrorCode(kCheckErrors, kErrorBusError, kErrorError);
    }

    if (systickTimeout(start_tick, timeout_ms)) {
        LL_I2C_GenerateStopCondition(descriptor);
        return createErrorCode(kCheckErrors, kErrorTimeout, kErrorError);
    }

    return kSuccessCode;
}
